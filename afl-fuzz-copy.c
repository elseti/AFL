...
FILE *fuzzFilePtr; // modification - file to store fuzz inputs
FILE *queueFilePtr; // modification - file to store fuzz inputs from queue ("interesting")
...
EXP_ST void destroy_queue(void) {
  ...
  queueFilePtr = fopen("afl-queue-output.txt", "a");

  if (queueFilePtr == NULL) {
    // Handle the case when file opening fails
...
static void write_to_testcase(void* mem, u32 len) {
  ...
  // modification
  fuzzFilePtr = fopen("afl-fuzz-output.txt", "a");
  if (fuzzFilePtr == NULL) {
    // Handle the case when file opening fails
    perror("Error opening file");
    return;
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  // printf("%s\n", *argv); // this outputs the exec name
...
static u8 fuzz_one(char** argv) {
  // fptr = fopen("afl-fuzz-output.txt", "a");
...
int main(int argc, char** argv) {
  ...
  // modification - clear file - put earlier on when start 
  fuzzFilePtr = fopen("afl-fuzz-output.txt", "w");
  if (fuzzFilePtr == NULL) {
      perror("Error opening file afl-fuzz-output.txt");
  }
  fclose(fuzzFilePtr);

  // modification - clear file - put earlier on when start 
  queueFilePtr = fopen("afl-queue-output.txt", "w");
  if (queueFilePtr == NULL) {
      perror("Error opening file afl-queue-output.txt");
  }
  fclose(queueFilePtr);
